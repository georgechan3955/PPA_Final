...
variablesgen <- c(med_income = "B19013_001E",  # Median household income
...
acs2010 <- get_acs(geography = "tract", variables = variablesgen, year = 2010, 
                   survey = "acs5", state = 06, county = 037, geometry = TRUE,output = "wide") %>% 
  select(-matches("M$"))
acs2015 <- get_acs(geography = "tract", variables = variablesgen, year = 2015, 
                   survey = "acs5", state = 06, county = 037, geometry = TRUE,,output = "wide") %>% 
  select(-matches("M$"))
...
datatract <- left_join(acs2009_nogeom, acs2010_nogeom, by = "GEOID") %>%
...
# Convert it back
data_sf <- st_as_sf(datatract, geometry = st_geometry(acs2009), crs = st_crs(acs2009))
...
studyingdata <- st_intersection(data_sf, la_city_boundary)
...
# Convert thresholds to a simpler format
thresholds_2009 <- as.data.frame(thresholds_2009)

# Filter data by thresholds to identify eligible tracts in 2009
data_sf <- data_sf %>%
  mutate(
    eligible_2009 = if_else(
...
# Calculate 40th percentile changes as new thresholds for changes
thresholds_2010 <- data_sf %>%
  summarise(
...
# Define gentrification based on the changes between 2010 and 2015
data_sf <- data_sf %>%
  mutate(
    gentrified = if_else(
      eligible_2009 & 
      home_value_change > thresholds_2010$top_60th_home_value_change &
      income_change > thresholds_2010$top_60th_income_change &
      education_change > thresholds_2010$top_60th_education_change,
      1, 0)
  )

# Assuming data_sf already contains the gentrified column
data_sf <- data_sf %>%
