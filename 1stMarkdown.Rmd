...
```{r}
tractgeography <- tractgeography %>%
  mutate(TOD = ifelse(intersection == 1, "TOD", "non-TOD"))

# Basic map with TOD classification
TOD <- ggplot(data = tractgeography) +
  geom_sf(aes(fill = TOD), color = "black", size = 0.1) +
  scale_fill_manual(values = c("TOD" = "#7892B5", "non-TOD" = "#D98481")) +
  labs(title = "TOD Area by Census Tract",
       fill = "TOD Status") +
  theme_void()

# Print the map
print(map)
ggsave("TOD.png", plot = TOD, bg = "transparent", width = 10, height = 8, dpi = 300)
```
...
# Convert acs2015 to a spatial dataframe and select relevant columns
datageographychi <- st_as_sf(acs2015_chicago, geometry = "geometry", crs = 4326) %>%
  select(GEOID, geometry)
...
# Transform datanew to match the CRS of boundary
datanewchi <- st_transform(datanew, st_crs(chicagoBoundary))
