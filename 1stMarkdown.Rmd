  ...
  gather(Variable, value, -gentrification) %>%
    ggplot(aes(gentrification, value, fill=gentrification)) + 
      ...
      facet_wrap(~Variable, scales = "free", ncol = 4, labeller= labeller(Variable = c(
                     `changeinpoverty` = "Change in Poverty from 2015 to 2020",
                     `changeinbachelor` = "Change in percent of people with at least a bachelor degree",
                     `changeinwhite` = "Change in percent of people with at least a bachelor degree
",
                     `pctBachelors20` = "Percent of people with at least a bachelor's degree in 2020",
                     `changein2544` = "Change in population aged from 25 to 44 per tract",
                     `houseprice20`= "Median house price per tract in 2020",
                      rent= "Median rent price per tract in 2015",
                     rent20= "Median rent price per tract in 2020",
                     changeinhouseprice= "Change in median house price per tract",
                     newhousingunit= "new housing units", 
                     incomeChange= "Income change from 2015 to 2020",
                     ForMig_Change= " Change in percentage of migration",
                     Density_Change= "Change in population density"))) +
  ...
  gather(Variable, Value, -Race) %>%
    ggplot(aes(Variable, Value, fill = Race)) +
      ...
      labs(title="Confusion matrix rates by race",
           subtitle = "10% threshold", x = "Outcome",y = "Rate") +
      theme_minimal() + theme(axis.text.x = element_text(angle = 0, hjust = 1)) 
```
