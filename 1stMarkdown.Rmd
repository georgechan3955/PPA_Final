...
gentrifyModel <- glm(gentrification ~ .,data=gentrifydataTrain %>% dplyr::select(gentrification, changeinpoverty,changeinbachelor, changeinwhite,pctBachelors20,pctBachelors,changein2544,houseprice20,rent,rent20,changeinhouseprice,newhousingunit),family="binomial"(link="logit"))
...
gentrifydata_clean <- gentrifydata %>%
  select(gentrification, changeinpoverty,changeinbachelor, changeinwhite,pctBachelors20,pctBachelors,changein2544,houseprice20,rent,rent20,changeinhouseprice,newhousingunit) %>%
  na.omit() 
...
customSummary <- function(data, lev = NULL, model = NULL) {
  ...
  prob <- ifelse(data[, "true"] >= 0.1, lev[2], lev[1])
...
trainIndex <- caret::createDataPartition(y = gentrifydatachi$NAME, p = 0.65, list = FALSE, times = 1)
...
gentrifyModelchi <- glm(gentrification ~ .,data=gentrifydatachiTrain %>% dplyr::select(gentrification, changeinpoverty,changeinbachelor, changeinwhite,pctBachelors20,pctBachelors, changein2544,houseprice20,rent,rent20,changeinhouseprice,newhousingunit),family="binomial"(link="logit"))
